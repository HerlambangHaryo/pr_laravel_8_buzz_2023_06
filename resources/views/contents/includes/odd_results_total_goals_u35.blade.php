<div>
    <span class="badge bg-gray-700 mt-2" >
        home - draw - away
        <br/>
        {{ $row->pre_result_total_goals_home_under_35 }} - {{ $row->pre_result_total_goals_draw_under_35 }} - {{ $row->pre_result_total_goals_away_under_35 }}
    </span>
</div>
        ...
        <a
            @if(!is_null($row->end_result_total_goals_home_under_35))
                href="{{route('Mybets.datacreate',[
                    ...
                    'betsapi_id'        => 1,
                    'value'             => 'Home',
                    'odd'               => $row->end_result_total_goals_home_under_35,
                ])}}"
            @endif
            target="_blank"
            class=" ms-2 me-2">
        ...
        <a
            @if(!is_null($row->end_result_total_goals_draw_under_35))
                href="{{route('Mybets.datacreate',[
                    ...
                    'betsapi_id'        => 2,
                    'value'             => 'Draw',
                    'odd'               => $row->end_result_total_goals_draw_under_35,
                ])}}"
            @endif
            target="_blank"
            class=" ms-2 me-2">
        ...
        <a
            @if(!is_null($row->end_result_total_goals_away_under_35))
                href="{{route('Mybets.datacreate',[
                    ...
                    'betsapi_id'        => 3,
                    'value'             => 'Away',
                    'odd'               => $row->end_result_total_goals_away_under_35,
                ])}}"
            @endif
            target="_blank"
            class=" ms-2 me-2">
